...
export function FeaturedDestinations({ branches = [] }: { branches?: StorefrontBranch[] }) {
  ...
  const items: CardItem[] = branches.map((branch) => ({
    key: branch.id,
    href: `/stores/${branch.slug}`,
    name: branch.name,
    region: branch.region,
    image: branch.imageUrl,
    products: branch.productCount,
  }));
  ...
  return (
    <section id="showrooms" ref={containerRef} className="relative overflow-hidden section-navy-band py-20 sm:py-32">
      ...
      <div className="relative mt-6 overflow-x-auto overflow-y-hidden hide-scrollbar sm:mt-8">
        <motion.div style={{ x }} className="flex w-max gap-4 px-4 sm:gap-6 sm:px-6">
          {doubled.map((item, i) => (
            <Link key={`${item.key}-${i}`} href={item.href} className="block shrink-0">
              <motion.div
                whileHover={{ scale: 1.03, y: -6 }}
                transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
                className="group relative h-[360px] w-[280px] overflow-hidden rounded-2xl shadow-xl ring-2 ring-white/20 transition-all hover:ring-cyan/50 sm:h-[480px] sm:w-[340px] sm:rounded-3xl md:h-[520px] md:w-[380px]"
              >
                <Image
                  src={item.image}
                  alt={item.name}
